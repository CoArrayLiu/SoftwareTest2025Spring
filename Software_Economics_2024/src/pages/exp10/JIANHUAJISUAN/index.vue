...
<template>
  ...
  <h3>2.项目现金流入</h3>
  <p class="content">
    项目的现金流入包括独立方案的年收入，其中，独立方案是指各工程项目方案的现金流是独立的、不相关，而且任何一个方案的采用与否都不会对是否采纳其他方案的决策产生影响。例如:阿里集团的淘宝和闲鱼，分别投资开发各自平台新产品方案，如果选择某个方案，不会影响到另一个方案，则这两个方案就是独立方案。请按照下列表格填写项目每年的现金流入：
  </p>
  ...
  <h3>5.项目指标</h3>
  <p class="content">
    根据所提取的财务评价所需基础数据进行项目财务分析，重在考察项目盈利能力是否能够满足要求。编制项目资本金现金流量表，计算项目资本金财务内部收益率IRR、净现值NPV以及动态投资回收期（年），考察项目资本金可获得的收益水平。
  </p>
  ...
  setup(props, { emit }) {
    ...
    const submit = () => {
      ...
      const data = {
        year: year.value,
        // 此处为实验数据
        flowInTable:toRaw(Input.value),
        flowOutTable:toRaw(Output.value),
        netCashFlow:toRaw(netCashFlow.value),
        indexTable :toRaw(Index.value),
