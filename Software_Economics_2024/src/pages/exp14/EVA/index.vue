...
<template>
  ...
  <div>
      ...
      <div>
        <a-form ref="planRef" name="plan_item" :model="plannedForm" @finish="onFinish">
          <a-space v-for="(timePoint, index) in plannedForm.timePoints" :key="timePoint.id"
            ...
            <a-form-item :name="['timePoints', index, 'startDay']" :rules="{
            required: true,
            message: '缺少开始日期',
          }" :label="`阶段${index + 1}`">
              <a-date-picker v-model:value="timePoint.startDay" placeholder="该阶段开始日期"
                @change="(date) => onChangeStart(date, index)"
                :disabled-date="(date) => disabledStart(date, index)"></a-date-picker>
            </a-form-item>
            <a-form-item :name="['timePoints', index, 'endDay']" :rules="{
            required: true,
            message: '缺少结束日期',
          }" style="margin-left: 100px">
              <a-date-picker v-model:value="timePoint.endDay" placeholder="该阶段结束日期"
                @change="(date) => onChangeEnd(date, index)"
                :disabled-date="(date) => disabledEnd(date, index)"></a-date-picker>
            </a-form-item>
            ...
            <a-form-item :name="['timePoints', index, 'cost']" :rules="{
            required: true,
            message: '缺少计划成本',
          ...
          <a-space v-for="(timePoint, index) in dynamicValidateForm.timePoints" :key="timePoint.id"
            ...
            message: '缺少开始日期',
              ...
              <a-date-picker v-model:value="timePoint.startDay" placeholder="该阶段开始日期"
                ...
                :disabled-date="(date) => disabledStart2(date, index)"></a-date-picker>
            </a-form-item>
            <a-form-item :name="['timePoints', index, 'endDay']" :rules="{
            required: true,
            message: '缺少结束日期',
              ...
              <a-date-picker v-model:value="timePoint.endDay" placeholder="该阶段结束日期"
                ...
                :disabled-date="(date) => disabledEnd2(date, index)"></a-date-picker>
            </a-form-item>
            <a-form-item :name="['timePoints', index, 'EV']" :rules="{
            required: true,
            message: '缺少实际工作量',
          }" style="margin-left: 110px">
    ...
    <div>
      <a-row :gutter="16">
        <a-col :span="12">
          <div ref="myChart" id="chart" style="width: 600px; height: 600px"></div>
        </a-col>
        <a-col :span="12">
          <div ref="myChart2" id="chart2" style="width: 600px; height: 600px"></div>
        </a-col>
      </a-row>
    </div>
    ...
    <a-textarea v-if="parameter.length !== 0" v-model:value="summary" placeholder="请在此处填写实验心得" show-count
      :maxlength="500" :rows="5" style="margin-top: 10px; margin-bottom: 30px">
    </a-textarea>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let v = dayPlan / day; */
      ...
      let totalSV = objEV[days] - objPV_days;
    ...
    const submit = () => {
      ...
      let imgUrl1 = ""
      let imgUrl2 = ""
      const experiment_id = "141"
      const { account } = useAccountStore()
      const { uploadImg } = useExperimentStore()

      const _plannedForm = plannedForm.timePoints.map((item) => {
        return {
          startDay: `${item.startDay.$y}-${item.startDay.$M + 1}-${item.startDay.$D}`,
          endDay: `${item.endDay.$y}-${item.endDay.$M + 1}-${item.endDay.$D}`,
          ...item
        }
      });
      const _dynamicValidateForm = dynamicValidateForm.timePoints.map((item) => {
        return {
          startDay: `${item.startDay.$y}-${item.startDay.$M + 1}-${item.startDay.$D}`,
          endDay: `${item.endDay.$y}-${item.endDay.$M + 1}-${item.endDay.$D}`,
          ...item
      ...
      html2canvas(document.getElementById("chart")).then(function (canvas) {
        canvas.toBlob(
          function (blob) {
            imgUrl1 = uploadImg(blob, experiment_id, account.email.slice(0, account.email.indexOf("@")), "eva1");
          });
      }).then(function () {
        html2canvas(document.getElementById("chart2")).then(function (canvas) {
          canvas.toBlob(
            function (blob) {
              imgUrl2 = uploadImg(blob, experiment_id, account.email.slice(0, account.email.indexOf("@")), "eva2");
              const data = {
                image: {
                  chart1: imgUrl1,
                  chart2: imgUrl2
                ...
                // 此处为实验数据
                plannedForm: _plannedForm,
                dynamicValidateForm: _dynamicValidateForm,
                // 基础指标
                parameter: parameter,
                // 各阶段SV、CV指标
                vDataSource: vDataSource,
                // 各阶段SPI、CPI指标
                piDataSource: piDataSource,
