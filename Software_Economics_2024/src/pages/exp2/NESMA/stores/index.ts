...
export const useStore = defineStore(
  ...
  () => {
    ...
    const C = computed(
      () => (index) =>
      (tableData7.value[index].C =
        (tableData7.value[index].A ? parseInt(tableData7.value[index].A) : 0) * parseInt(tableData7.value[index].B) +
        '')
    );
    // 
    // 简单功能点数
    ...
    //数据在填写过程中自动保存，此函数并没有用 
    // const saveData = () => {  

    // }
    return {
      ILF, EIF, ETM, ETE, selectMethod, indicativeAlg, tableData6, summary,
      tableData5, tableData7, c, f, i, unchanged, SUM, VAF, ALL, C, clear
    };
  },
  {
