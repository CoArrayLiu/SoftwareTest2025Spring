<template>
        ...
        <a-table bordered :data-source="table3" :columns="columns" :pagination="false">
            <template #bodyCell="{ column, text, record }">
                ...
                <template v-else-if="column.dataIndex === 'operation'">
                    ...
                    <a-popconfirm v-if="table3.length" title="Sure to delete?" @confirm="onDelete(record.key)">
                        <a>Delete</a>
                    </a-popconfirm>
                </template>
            </template>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const data = storeToRefs(MaxmaxDataStore);
        ...
        const start = () => {
            ...
            table3.value.forEach(function (item) {
        ...
        const edit = (key: string) => {
            editableData[key] = cloneDeep(table3.value.filter(item => key === item.key)[0]);
        };
        const save = (key: string) => {
            Object.assign(table3.value.filter(item => key === item.key)[0], editableData[key]);
        ...
        const onDelete = (key: string) => {
            table3.value = table3.value.filter(item => item.key !== "0");
            table3.value = table3.value.filter(item => item.key !== key);
            emit('update', table3.value)
        };
        const handleAdd = () => {
            onDelete("0")
